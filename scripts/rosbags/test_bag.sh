	...
	then
		...
		then
			...
			then
				...
				cp $var $HOME/900RobotCode/practice$(basename $var)
			...
			then
				...
				cp $HOME/900RobotCode/$practice_merge_file $HOME/900RobotCode/unmerged_${practice_merge_file}
				~/900RobotCode/zebROS_ws/src/rosbag_scripts/scripts/merge_bagfiles.py $practice_merge_file $var unmerged_$practice_merge_file -v
		...
		then 
			...
			then
				...
				cp $var $HOME/900RobotCode/match0$(basename $var)
				continue
			fi
			...
			then 
				...
				~/900RobotCode/zebROS_ws/src/rosbag_scripts/scripts/merge_bagfiles.py ${bag_name}_${count}.bag $var ${bag_name}*.bag -v
				rm ${bag_name}_$(( $count - 1)).bag
				rm ${bag_name}.bag
				count=$(( $count + 1 ))
			else 
				echo Renaming bag file to ${bag_name}.bag
				cp $var $HOME/900RobotCode/${bag_name}.bag
			fi
		else
			echo This does not have alliance data -- renaming to prematch${var}
			cp $var $HOME/900RobotCode/prematch$(basename $var)
		fi
	else
